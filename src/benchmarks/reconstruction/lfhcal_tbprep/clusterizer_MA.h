...
struct towersStrct {
  towersStrct()
      ...
      , tower_ROtype(-1)
      , ltpr(0.0)
      , ltrbit(0) {}  // Readout type for test beam format
  float energy;
  float time;
  float posx;
  float posy;
  ...
  int tower_ROtype;  // Readout type: 0=Undef, 1=Hgcroc, 2=Caen (from test beam format)
  float ltpr;        // Local trigger primitive
